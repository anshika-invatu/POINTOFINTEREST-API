...
describe('Delete PointOfInterest', () => {
    before(async () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/point-of-interest/123-abc`, {
                json: true,
                headers: {
    ...
    it('should throw 404 error if the doc of specified id not exist', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/point-of-interest/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY
                }
    ...
    it('should delete the document when all validation passes', async () => {
        const result = await request.delete(`${helpers.API_URL}/api/v1/point-of-interest/${samplePointOfInterest._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
